jQuery(document).ready(function() {
	...
	if( !jQuery( ".senses-info" ).length ) {
		...
		jQuery('.senses-info').append('<a style="width: 80%; margin: 5px auto 5px auto; display: block; text-align: center;" href="http://www.shapedpixels.com/setup-senses-lite/" class="button" target="_blank">{setup}</a>'.replace('{setup}', sensesliteCustomizerObject.setup));
		...
		jQuery('.senses-info').append('<a style="width: 80%; margin: 5px auto 5px auto; display: block; text-align: center;" href="http://www.shapedpixels.com/senses" class="button" target="_blank">{pro}</a>'.replace('{pro}',sensesliteCustomizerObject.pro));
